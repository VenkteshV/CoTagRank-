...
word_to_idf = joblib.load(dir_path+"/../../data/Models/Unsupervised/lda/word2idf_semeval2017")
tf_idf_vectorizer = joblib.load(dir_path+"/../../data/Models/Unsupervised/lda/tf_idf_vectorizer_semeval2017")
stemmer = SnowballStemmer("porter")
class CoTagRankPlusPlus:
    ...
    def RankPhrases(self, text_emb, phrases, phrase_embs, top_n=10, alias_threshold=0.8, highlight = False, og_doc_text=None):
        ...
        for (keyword, start_pos, end_pos), score, manhattan in zip(top_phrases, document_relevance, doc_rel_manhaatan_dist):
            ...
            relevance_dict[keyword] = score * phrase_idf_score  * (np.exp(1 - (len(keyword.split())/2))) #(1/len(keyword.split()))
...
# sim_importance * score + (1-sim_importance) * (1.0/(0.000001+start_pos))
        ...
        graph.add_weighted_edges_from(
            [(v[0], u[0], ( 0.8 *(((np.dot(phrase_to_embedding[v[0]], phrase_to_embedding[u[0]])/((np.linalg.norm(phrase_to_embedding[v[0]]) * np.linalg.norm(phrase_to_embedding[u[0]]))+1e-07)) - minx) / (maxx-minx) ) + 0.2 * (1/(1.0+u[1])) ) ) for v in top_phrases for u in top_phrases if u != v])       
