...
class CoTagRankBERTTopical(object):
    def __init__(self, numOfKeywords, pathData, dataset_name, normalization):
        super().__init__()
        self.__lan = getlanguage(pathData + "/Datasets/" + dataset_name)
        self.__numOfKeywords = numOfKeywords
        ...
        self.model = init_keyword_extractor(read_json(dir_path+'/config/conceptrank_bert_topical.json'))
    ...
    def runSingleDoc(self, doc, lists, text = None, highlight=None, expand=False):
        try:
            ...
            # extract keywords
            if expand:
                keywords, phrase_with_positions, color_map = self.model.run(doc_text, texts, method = "CoTagTopical", lists=lists, highlight=highlight, expand=expand)
            else:
                keywords, phrase_with_positions = self.model.run(doc_text, texts, method = "CoTagTopical", lists=lists, highlight=highlight, expand=expand)
...
class CoTagRankPositionalEmbeddingUSE(object):
    def __init__(self, numOfKeywords, pathData, dataset_name, normalization):
        super().__init__()
        self.__lan = getlanguage(pathData + "/Datasets/" + dataset_name)
        self.__numOfKeywords = numOfKeywords
        ...
        self.model = init_keyword_extractor(read_json(dir_path+'/config/conceptrank_positional_USE.json'))
