...
def keyword_extraction(expand=False):
    for algorithm in ListOfAlgorithms:
        ...
        for i in range(len(ListOfDatasets)):
            ...
            elif algorithm == "CoTagRankBERTTopical":
                print("pathData", pathData)
                CoTagRankBERTTopical_object = CoTagRankBERTTopical(numOfKeyphrases, pathData, dataset_name,
                                                                        normalization)
                                                        
                CoTagRankBERTTopical_object.ExtractKeyphrases(expand=expand)
                CoTagRankBERTTopical_object.Convert2Trec_Eval(EvaluationStemming)

            elif algorithm == "CoTagRankPositionalEmbeddingUSE":
                CoTagRankPositionalEmbeddingUSE_object = CoTagRankPositionalEmbeddingUSE(numOfKeyphrases, pathData, dataset_name,
                                                                        normalization)
